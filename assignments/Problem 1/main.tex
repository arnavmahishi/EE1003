...
\begin{align}
    ...
    \implies \vec{V}^{\prime}\brak{t}&=A\vec{V}\brak{t}
\end{align}
Where $\vec{V}\brak{t}$ is the vector $\myvec{1\\y_{0}\\y_{1}\\y_{2}\\ \vdots\\y_{n-1}}$ at a $t$.
At any $N$, by defining derivative:
\begin{align}
	y_N^{\prime}\brak{t} &= \lim_{h\to 0}\frac{y_N\brak{t+h} - y_N\brak{t}}{h}\\
	y_N\brak{t+h} &= y_N\brak{t} + hy_N^{\prime}\brak{t}\\
    \implies y_0\brak{t+h} &= y_0\brak{t} + hy_0^{\prime}\brak{t}\\
    \implies y_1\brak{t+h} &= y_1\brak{t} + hy_1^{\prime}\brak{t}\\
    \implies y_2\brak{t+h} &= y_2\brak{t} + hy_2^{\prime}\brak{t}\\
\vdots\quad\vdots\quad\vdots\quad\vdots&\quad\vdots\quad\vdots\quad\vdots\quad\vdots\quad\vdots\\
    \implies y_{n-1}\brak{t+h} &= y_{n-1}\brak{t} + hy_{n-1}^{\prime}\brak{t}\\
    \implies 
\vec{V}\brak{t+h}&=\vec{V}\brak{t}+h\brak{A\vec{V}\brak{t}}
\end{align}
When $t$ ranges from $t_o$ to $t_f$ in increments of $h$, discretizing the steps gives us all $\vec{V}\brak{x}$ from $t_o$ to $t_f$ in increments of $h$
Record the $y_0$ for each $\vec{V}\brak{x}$ we got and then plot the graph. The result will be as given below.
\begin{figure}[h!]
   \centering
